...
def comic_list_filtered_api_view(request):
    # Filtrar los comics con precio mayor o igual a 5.00
    filtered_comics = Comic.objects.filter(price__gte=5.00).values('id', 'title', 'price')
    
    # Devolver la respuesta directamente
    return Response(data=filtered_comics, status=status.HTTP_200_OK)